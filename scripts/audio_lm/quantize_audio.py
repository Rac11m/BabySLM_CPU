...
def quantize_file(file_path, cpc_feature_function, clusterModule, device):
    cFeatures = cpc_feature_function(file_path).to(device)

    nGroups = cFeatures.size(-1) // clusterModule.Ck.size(-1)

    cFeatures = cFeatures.view(1, -1, clusterModule.Ck.size(-1))

    if cFeatures.size(1) > 50000:
        clusterModule_cpu = clusterModule.to("cpu")
        qFeatures = torch.argmin(clusterModule_cpu(cFeatures.to("cpu")), dim=-1)
        clusterModule.to(device)
    else:
    ...
    quantLine = ",".join(
        ["-".join([str(i) for i in item]) for item in qFeatures.reshape(-1, nGroups)]
    )
...
def main(argv):
    ...
    seqNames, _ = findAllSeqs(
        args.pathDB, speaker_level=0, extension=args.file_extension, loadCache=True
    )
    ...
    if len(seqNames) == 0 or not os.path.splitext(seqNames[0][1])[1].endswith(
        ...
        seqNames, _ = findAllSeqs(
            args.pathDB, speaker_level=0, extension=args.file_extension, loadCache=False
        )

    print(f"Done! Found {len(seqNames)} files!")

    if args.debug:
        seqNames = seqNames[:20]
        print("Debug mode: using 20 files")

    Path(args.pathOutputDir).mkdir(parents=True, exist_ok=True)
    writeArgs(os.path.join(args.pathOutputDir, "_info_args.json"), args)

    outputFile = os.path.join(args.pathOutputDir, "quantized_outputs.txt")
    assert not os.path.exists(outputFile) or args.resume, "Output exists, use --resume"

    pathConfig = args.pathClusteringCheckpoint[:-3] + "_args.json"
    if not os.path.exists(pathConfig):
        pathConfig = os.path.join(
            os.path.dirname(args.pathClusteringCheckpoint), "args.json"
        )

    clustering_args = readArgs(pathConfig)
    print(json.dumps(vars(clustering_args), indent=4, sort_keys=True))

    print("Loading ClusterModule...")
    clusterModule = loadClusterModule(args.pathClusteringCheckpoint).to(device)
    ...
    def cpc_feature_function(x):
        if not args.nobatch:
            ...
            return buildFeature(featureMaker, x, seqNorm=False, strict=args.strict)
    ...
    for index, vals in enumerate(seqNames):
        bar.update(index)
        file_path = os.path.join(args.pathDB, vals[1])
        quantLine = quantize_file(
            file_path, cpc_feature_function, clusterModule, device
        )
        file_name = os.path.splitext(os.path.basename(file_path))[0]
        f.write(f"{file_name}\t{quantLine}\n")

    bar.finish()
    f.close()
    print(f"Done {len(seqNames)} files in {time() - start_time:.2f}s")
...
#         clustering_args.pathCheckpoint = os.path.join(os.path.dirname(os.path.abspath(args.pathClusteringCheckpoint)),
#                                                       clustering_args.pathCheckpoint)
