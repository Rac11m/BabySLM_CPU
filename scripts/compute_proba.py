...
def parseArgs(argv):
    ...
    parser.add_argument(
        "--dict_path",
        type=str,
        help="The path to the folder where to find dict.txt used to train the "
    ...
    parser.add_argument(
        "--debug", action="store_true", help="If True, will only process 10 sequences."
    )
    parser.add_argument(
        "--mode",
        type=str,
...
def main(argv):
    ...
    if args.phonemize:
        if args.input_path == "lexical":
            raise ValueError(
                "--phonemize flag cant be activated when evaluating on lexicon."
            )
    ...
    if args.text:
        stimuli = load_stimuli_text(
            args.input_path, args.mode, args.debug, args.phonemize
        )
    else:
        stimuli = load_quantized_units(args.input_path, args.mode, args.debug)

    prob_extractor = LstmProbExtractor(
        model_path=args.model_path,
        dict_path=args.dict_path,
        out_path=out_path,
        batch_size=args.batch_size,
        bpe_encode=args.bpe_encode,
        bos_eos=args.bos_eos,
    ...
    for data, data_name in zip(stimuli, args.mode):
        ...
        out_file = (
            args.model_path.parent / type_data / "tmp" / type_task / f"{data_name}.txt"
        )
